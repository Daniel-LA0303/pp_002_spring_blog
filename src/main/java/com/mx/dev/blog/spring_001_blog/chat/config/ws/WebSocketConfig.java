...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
	...
	// view in console of connections
	/*
	 ...
	 * channel) { StompHeaderAccessor accessor = StompHeaderAccessor.wrap(message);
	 * 
	 * if (StompCommand.CONNECT.equals(accessor.getCommand())) { int current =
	 * activeConnections.incrementAndGet();
	 * System.out.println("User connected. Active connections: " + current); }
	 * 
	 * if (StompCommand.DISCONNECT.equals(accessor.getCommand())) { int current =
	...
	public void registerStompEndpoints(StompEndpointRegistry registry) {
		...
		registry.addEndpoint("/ws")
				.setAllowedOrigins("http://localhost:5173", "http://localhost:4200/", "http://192.168.100.3:5173")
				.withSockJS();
	}
